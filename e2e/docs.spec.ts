...
import { loadTestEnv, cleanupTestDb, verifyTestDb, setupPageWithUser, createUniqueUser } from './test-utils';
...
test.describe('Documents Feature', () => {
  ...
  test('should display New Doc button in sidebar', async ({ page }) => {
    ...
    // Check if New Doc button is visible
    const newDocBtn = page.locator('.new-doc-btn');
    await expect(newDocBtn).toBeVisible({ timeout: 10000 });
    await expect(newDocBtn).toContainText('Doc');
  });

  test('should create a new document and navigate to it', async ({ page }) => {
    // Setup unique user for this test
    await setupPageWithUser(page);
  ...
  test('should delete document from sidebar', async ({ page }) => {
    // Setup unique user for this test
    const user = await setupPageWithUser(page);
    ...
    // Find the doc item and hover to show delete button
    const docItem = page.locator('.chat-item').filter({ hasText: 'Delete Me Doc' });
    await expect(docItem).toBeVisible({ timeout: 5000 });
    await docItem.hover();

    // Click delete button
    const deleteBtn = docItem.locator('.chat-item-delete');
    ...
    // Document should be gone from sidebar
