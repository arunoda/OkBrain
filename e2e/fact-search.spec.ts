...
test.describe('Fact Search', () => {
  ...
  // Extract facts once for all tests
  test('setup: extract facts from conversations', async ({ page, request }) => {
    test.setTimeout(120_000);

    sharedUser = await setupPageWithUser(page);
    ...
    const headers = { Cookie: `auth-token=${sharedUser.token}` };
