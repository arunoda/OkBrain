...
import { loadTestEnv, cleanupTestDb, waitForApiResponse, verifyTestDb, setupPageWithUser, waitForChatCompletion } from './test-utils';
...
test.describe('Gemini File Upload', () => {
  ...
  test('should upload image, ask question about it, and show attachment in message', async ({ page }) => {
    ...
    // Verify file badge is shown in the user message (shows "📎 1 file")
    const fileBadge = page.locator('.message-file-badge');
    await expect(fileBadge).toBeVisible({ timeout: 5000 });
    await expect(fileBadge).toContainText('1 file');
...
test.describe('Gemini Status and Thinking', () => {
  ...
  test('should show thinking indicator and allow expanding thoughts when THINK is enabled', async ({ page }) => {
    // NOTE: This test verifies the thinking UI when the Gemini API returns thinking data.
    // With THINK enabled (thinkingLevel: HIGH), Gemini should provide thinking.
    ...
    // 2. Select Gemini (default)
    const aiProvider = page.locator('#ai-provider');
    await aiProvider.selectOption('gemini');

    // 3. Ensure THINK checkbox is checked
    ...
    // 4. Try multiple prompts to increase likelihood of getting thinking response
    const prompts = [
      'Think step by step: what is the 10th prime number? Just think about it, then give the answer.',
      'Solve this step by step: If a train travels 60 miles in 1 hour, how far will it travel in 2.5 hours? Think through it.',
      'Think carefully: What is 15 * 23? Show your thinking process.',
    ];
    ...
    for (let attempt = 0; attempt < prompts.length; attempt++) {
      ...
      assistantMessage = page.locator('.message.assistant').last();
      ...
      // Check if thinking indicator appeared
      const thinkingIndicator = page.locator('text=Thinking').first();
      const thoughtIndicator = page.locator('text=/Thought for \\d+s/i').first();

      const hasThinking = await thinkingIndicator.isVisible().catch(() => false);
      const hasThought = await thoughtIndicator.isVisible().catch(() => false);

      if (hasThinking || hasThought) {
        thoughtIndicatorFound = true;
        console.log(`Thinking indicator found on attempt ${attempt + 1}`);
        break;
      }

      console.log(`No thinking indicator on attempt ${attempt + 1}, trying next prompt...`);
    }

    // If we never got thinking after all attempts, skip the test
    if (!thoughtIndicatorFound) {
      console.log('Gemini did not provide thinking data after multiple attempts. Skipping test.');
      test.skip();
      return;
    }

    // Now test the thinking UI functionality
    const thoughtIndicator = page.locator('text=/Thought for \\d+s/i').last();
    ...
    assistantMessage = page.locator('.message.assistant').last();
    const thoughtsContainer = assistantMessage.locator('.content-styles').first();
...
test.describe('SSR Resume', () => {
  ...
  test('should preserve streaming content when page is reloaded mid-stream', async ({ page }) => {
    ...
    const input = page.locator('textarea[placeholder="Ask me anything..."]');
    await input.fill('What is the current weather in Colombo, Sri Lanka? Please think about this carefully and provide a detailed response with the temperature, conditions, and any weather advisories.');
    ...
    await page.waitForFunction(
      () => {
        const typingIndicator = document.querySelector('.message.assistant.streaming .typing-indicator');
        const msgText = document.querySelector('.message.assistant.streaming .message-text');
    ...
    const typingIndicatorBefore = page.locator('.message.assistant.streaming .typing-indicator');
    const hasIndicatorBefore = await typingIndicatorBefore.isVisible().catch(() => false);
    if (hasIndicatorBefore) {
      const indicatorTextBefore = await typingIndicatorBefore.textContent();
    ...
    console.log('Content before reload length:', contentBeforeReload?.length);
    ...
    const typingIndicator = page.locator('.message.assistant .typing-indicator');
    const isStillStreaming = await typingIndicator.isVisible().catch(() => false);

    if (isStillStreaming) {
      const indicatorText = await typingIndicator.textContent();
      console.log('Typing indicator visible after reload with text:', indicatorText);
      ...
      console.log('Streaming already completed before we could check the indicator after reload');
    }

    // Get content after reload
    const reloadedMessageText = page.locator('.message.assistant .message-text').last();
    ...
    const hasWeatherContent = finalContent?.toLowerCase().includes('weather') ||
