...
test.describe('Chat Application', () => {
  ...
  test('should delete conversation with confirmation', async ({ page }) => {
    ...
    // Find delete button - get the specific conversation item
    const chatItems = page.locator('.chat-item');
    ...
    const deleteButton = chatItem.locator('.chat-item-delete');
    ...
    await expect(chatItem).toBeVisible({ timeout: 2000 });
    ...
    // Check conversation is removed - verify count decreased
    const remainingItems = page.locator('.chat-item');
    await page.waitForTimeout(500);
    const remainingCount = await remainingItems.count();

    // If there were multiple conversations, count should decrease
    // If it was the only one, the chat history section might be empty
    if (chatItemCount > 1) {
      expect(remainingCount).toBeLessThan(chatItemCount);
    } else {
      // If it was the only conversation, check that chat history is empty or item is gone
      const itemStillExists = await chatItem.isVisible().catch(() => false);
  ...
  test('should allow sending message after cancelling first message', async ({ page }) => {
    ...
    // The cancelled message should be restored in the input
    await expect(input).toHaveValue('First message to cancel');

    // URL should be back to home (not stuck on /chat/deleted-id)
    ...
    // Should get an assistant response this time
