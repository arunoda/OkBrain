...
export async function getDocument(
  ...
  await ensureInitialized();
  const result = await dbWrapper.prepare("SELECT * FROM documents WHERE id = ? AND user_id = ?").get(id, userId);
...
export async function updateDocumentTitle(
  ...
  await ensureInitialized();
  await dbWrapper.prepare(`
    UPDATE documents SET title = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?
  `).run(title, id, userId);
}
...
export async function getDocumentsByFolder(
  ...
  const results = await dbWrapper.prepare(
    "SELECT * FROM documents WHERE folder_id = ? AND user_id = ? ORDER BY updated_at DESC"
  ).all(folderId, userId);
  return results as Document[];
}
...
export async function getDocumentConversations(
  ...
  const results = await dbWrapper.prepare(`
    ...
    WHERE cd.document_id = ? AND c.user_id = ?
