...
function getDbPath(): string {
  ...
  if (process.env.TEST_MODE === 'true' || process.env.TEST_MODE === '1') {
    return path.join(process.cwd(), "brain.test.db");
  }
  ...
  return process.env.DATABASE_URL?.replace('file:', '') || path.join(process.cwd(), "brain.db");
}
...
function getDb(): Database.Database {
  ...
  const normalizedCurrentPath = path.resolve(currentDbPath);
  const normalizedCachedPath = dbPath ? path.resolve(dbPath) : null;
