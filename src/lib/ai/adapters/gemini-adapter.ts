...
// Pricing per 1M tokens (USD) — output price includes thinking tokens
const GEMINI_PRICING: Record<string, { input: number; cachedInput: number; output: number }> = {
  'flash': { input: 0.50, cachedInput: 0.05, output: 3.00 },
  'pro':   { input: 2.00, cachedInput: 0.20, output: 12.00 },
};

function getModelPricing(modelName: string) {
  if (modelName.includes('pro')) return GEMINI_PRICING['pro'];
  return GEMINI_PRICING['flash'];
}

export function calculateGeminiCost(usage: any, modelName: string) {
...
export function logGeminiUsage(label: string, usage: any, modelName: string) {
  const cost = calculateGeminiCost(usage, modelName);
...
export class GeminiProvider implements AIProvider {
  ...
  constructor(apiKey: string, modelName: string = "gemini-3-flash-preview", displayName?: string) {
  ...
  async generateStream(
    ...
    // Use custom tools (Google Maps, internet_search, etc.)
    const activeTools: Tool[] = toGeminiTools(allTools);
  ...
  private async generateStreamLoop(
    ...
    if (isThinkingModel || isGemini3) {
      config.thinkingConfig = {
        ...
        thinkingLevel: options?.thinking ? 'HIGH' : 'LOW'
      };
    }
    ...
    if (lastUsageMetadata) {
      logGeminiUsage('Gemini', lastUsageMetadata, this.modelName);
    }
    ...
    if (functionCallPart && functionCallPart.functionCall) {
      ...
      return this.generateStreamLoop(contents, systemInstruction, tools, onChunk, options, client, recursionDepth + 1);
    }
