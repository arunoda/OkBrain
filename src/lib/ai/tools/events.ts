...
async function executeSearchEvents(args: any): Promise<any> {
  try {
    ...
    const events = await searchEvents(userId, args.query);
    return {
      events,
      count: events.length
    };
...
async function executeGetEventsByDateRange(args: any): Promise<any> {
  try {
    const userId = getUserId();
    const events = await getEventsByDateRange(userId, args.start_date, args.end_date);
    return {
      events,
      count: events.length,
...
async function executeGetUpcomingEvents(args: any): Promise<any> {
  try {
    ...
    const events = await getUpcomingEvents(userId, Math.min(limit, 50));
    return {
      events,
      count: events.length
    };
...
async function executeGetPastEvents(args: any): Promise<any> {
  try {
    ...
    const events = await getPastEvents(userId, Math.min(limit, 50));
    return {
      events,
      count: events.length
    };
...
async function executeCreateEvent(args: any): Promise<any> {
  try {
    ...
    return {
      ...
      message: `Created ${createdEvents.length} event(s)${errors.length > 0 ? ` with ${errors.length} error(s)` : ''}`
    };
  } catch (error: any) {
...
const updateEventDefinition: ToolDefinition = {
  name: "update_event",
  description: "Update one or multiple existing events. This requires user confirmation. TIMEZONE HANDLING: ALWAYS store times in UTC (ISO 8601 with 'Z' suffix). When user provides time without timezone, interpret it in their current location's timezone and convert to UTC. When displaying times back to user, convert from UTC to their local timezone. RECURRENCE: Supports weekly and monthly recurring events.",
  parameters: {
    type: "OBJECT",
    properties: {
      events: {
...
async function executeUpdateEvent(args: any): Promise<any> {
  try {
    ...
    return {
      ...
      errors: errors.length > 0 ? errors : undefined,
      message: `Updated ${updatedEvents.length} event(s)${errors.length > 0 ? ` with ${errors.length} error(s)` : ''}`
    };
  } catch (error: any) {
...
const deleteEventDefinition: ToolDefinition = {
  name: "delete_event",
  description: "Delete one or multiple events. This requires user confirmation. Use this when the user wants to remove event(s).",
  parameters: {
    type: "OBJECT",
    properties: {
      event_ids: {
