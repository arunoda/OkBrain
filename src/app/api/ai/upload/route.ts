...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validTypes.includes(file.type)) {
      return NextResponse.json(
        { error: "Invalid file type. Only images (JPEG, PNG, GIF, WebP) and PDFs are supported." },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const uploaded = await uploadFileForModel(modelId, tempFilePath, file.type, file.name);
